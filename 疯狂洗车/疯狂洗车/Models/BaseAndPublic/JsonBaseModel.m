...
- (id)initWithDictionary:(NSDictionary *)jsonDic
{
    ...
    if (self != nil)
    {
        ...
        for ( i=0; i < propertyCount; i++ )
        {
            ...
            {
                id object = [[NSClassFromString(propertyKeyName) alloc] initWithDictionary:[jsonDic valueForKey:propertyKeyName]];
            ...
            else if ([[jsonDic valueForKey:propertyKeyName] isKindOfClass:[NSArray class]])
            {
                ...
                for (id object in [jsonDic valueForKey:propertyKeyName])
                {
                    ...
                    {
                        [arr addObject:[[NSClassFromString(propertyKeyName) alloc] initWithDictionary:object]];
                    }
                    else
                    {
