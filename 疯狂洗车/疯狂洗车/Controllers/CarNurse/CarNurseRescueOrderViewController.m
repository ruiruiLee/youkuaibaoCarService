...
@implementation CarNurseRescueOrderViewController
...
- (void)viewDidLoad
{
    ...
    if ([self.serviceModel.service_type isEqualToString:@"6"])
    {
        [self setTitle:@"速援"];
    }
    else
...
- (void)setUpOrderView
{
    ...
    NSString *carTitle = [NSString stringWithFormat:@"[%@] %@·%@ %@ %@",[_serviceCar.car_type isEqualToString:@"1"]?@"轿车" : @"SUV", [_serviceCar.car_no substringWithRange:NSMakeRange(0, 2)],[_serviceCar.car_no substringWithRange:NSMakeRange(2, _serviceCar.car_no.length-2)],_serviceCar.car_brand == nil ?@"无品牌信息":_serviceCar.car_brand,_serviceCar.car_xilie == nil?@"":_serviceCar.car_xilie ];
    _serviceCarLabel.text = [NSString stringWithFormat:@"%@",carTitle];
    
    if ([self.serviceModel.service_type isEqualToString:@"6"])
    {
        _serviceTypeAndWayLabel.text = [NSString stringWithFormat:@"%@ - 速援",self.serviceModel.service_name];
    }
    else
    {
        _serviceTypeAndWayLabel.text = [NSString stringWithFormat:@"%@ - 道路救援",self.serviceModel.service_name];
    }
