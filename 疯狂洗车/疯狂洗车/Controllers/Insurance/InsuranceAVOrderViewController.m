//
//  InsuranceAVOrderViewController.m
//  
//
...
@interface InsuranceAVOrderViewController ()
...
@implementation InsuranceAVOrderViewController
...
- (void)getInformationFromService
{
    ...
    NSString *urlString = @"carWash/service/carReview";
...
{
    if (self.carReviewModel.code_count.intValue > 0 && _userInfo.member_id != nil)
    {
        CarReviewModel *model = self.carReviewModel;
        _ticketEnable = YES;
    ...
    else if (_userInfo.member_id != nil)
    {
        [self loadMoreCars];
    }
    else
...
- (UITableViewCell*)tableView:(UITableView *)tableView myCarCellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (_myCarsArray.count > 1)
    {
        ...
        else
        {
            ...
            [cell setDisplayInfo:carTitle andNewPrice:_carReviewModel.member_price andOldPrice:_carReviewModel.original_price];
            if (_selectedCar != nil)
            {
    ...
    if (indexPath.section == 3)
    {
        if (indexPath.row == 0)
        {
            ...
            if ([_ticketName isEqualToString:@""] || _ticketName == nil || self.selectTicketModel == nil)
            {
                ...
                cell.ticketNumberLabel.text = [NSString stringWithFormat:@"%d张可用",_carReviewModel.code_count.intValue];
    ...
    if (indexPath.section == 3)
    {
        ...
        if (indexPath.row == 0)
        {
            ...
            else if (_ticketSelected || self.defaultTicketModel == nil)
            {
                ...
                viewController.delegate = self;
                viewController.targetCarWashID = self.carReviewModel.car_wash_id;
...
- (void)didIntroductionButtonTouched
{
    ...
    viewController.isAVService = YES;
    [self.navigationController pushViewController:viewController animated:YES];
}
...
- (void)updateDisplayPayPrice
{
    ...
    totalMoney = _carReviewModel.member_price.floatValue;
...
- (IBAction)didOrderPayButtonTouch:(id)sender
{
    ...
    if ([_addressString isEqualToString:@""] || _addressString == nil || _serviceLocation == nil)
    {
...
- (void)showAlertViewBeforePay
{
    ...
    payMoney = _carReviewModel.member_price.floatValue;
...
- (void)alertView:(UIAlertView *)alertView didDismissWithButtonIndex:(NSInteger)buttonIndex
{
    ...
    {
        [[UIApplication sharedApplication] openURL:[NSURL URLWithString:[NSString stringWithFormat:@"tel:%@",self.carReviewModel.contact_phone]]];
    }
    ...
    if (alertView.tag == 530 && buttonIndex == 1)
    {
        ...
        [submitDic setObject:_selectedCar.car_id
                      forKey:@"car_id"];
        [submitDic setObject:_carReviewModel.car_wash_id
                      forKey:@"car_wash_id"];
        [submitDic setObject:_carReviewModel.service_type
                      forKey:@"service_type"];
        ...
        [submitDic setObject:_carReviewModel.service_id
                      forKey:@"service_id"];
        
        [submitDic setObject:@"0"
                      forKey:@"is_super"];
        [submitDic setObject:_orderTimeString forKey:@"service_time"];
        [submitDic setObject:@2 forKey:@"service_mode"];
        [submitDic setObject:_addressString forKey:@"service_addr"];
...
//
        ...
        {
            [submitDic setObject:_moreRequestString forKey:@"requiry"];
        }
        
        float ticketMoney = 0;
        float yueMoney = 0;
        float payMoney = _carReviewModel.member_price.floatValue;
        float thirdPayMoney = 0;
        ...
        if (ticketMoney+yueMoney < _carReviewModel.member_price.floatValue)
        {
            thirdPayMoney = payMoney - ticketMoney - yueMoney;
