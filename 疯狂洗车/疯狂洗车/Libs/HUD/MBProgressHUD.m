...
+ (MB_INSTANCETYPE)showHUDWithText:(NSString *)text to:(UIView*)view animated:(BOOL)animated
{
    MBProgressHUD *hud = [[self alloc] initWithFrame:CGRectMake(0, view.bounds.size.height-80, 320, 80)];
//    UIView *customView = MB_AUTORELEASE([[UIImageView alloc] initWithImage:[UIImage imageNamed:@"37x-Checkmark.png"]]);
...
//    
	...
	[hud show:animated];
    [hud performSelector:@selector(hide:)
              withObject:@YES
              afterDelay:2.0];
	return MB_AUTORELEASE(hud);
}

+ (MB_INSTANCETYPE)showHUDWithText:(NSString *)text animated:(BOOL)animated
{
    MBProgressHUD *hud = [[self alloc] initWithView:[[UIApplication sharedApplication] windows][0]];
    UIView *customView = MB_AUTORELEASE([[UIImageView alloc] initWithImage:[UIImage imageNamed:@"37x-Checkmark.png"]]);
    hud.customView = customView;
